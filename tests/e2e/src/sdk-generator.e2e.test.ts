...
async function writeCatalog(productionRules: string, catalogPath: string): Promise<void> {
  await writeFile(catalogPath, buildCatalog(productionRules));
}

async function compileAst(catalogDir: string, astFile: string): Promise<void> {
...
async function generateSdk(catalogDir: string, outputDir: string): Promise<void> {
  const result = runCliCommand(
...
async function setupGeneratedSdk(sdkDir: string): Promise<void> {
  ...
  try {
    ...
    const errorOutput =
      tscError.stdout?.toString() || tscError.stderr?.toString() || tscError.message || 'Unknown error';
